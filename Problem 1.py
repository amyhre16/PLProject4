...
for department in { d[9] for d in s_emp[1::] }: print (lambda dept_id, avgSal: [dept_id, avgSal])(department, (lambda l: round(sum(l) / len(l), 2))(map(float, [ e[5] for e in s_emp[1::] if e[9] == department ])))

# 8. select dept_id, avg(salary) from s_emp group by dept_id having avg(salary) < 1500
print "\nselect dept_id, avg(salary) from s_emp gropu by dept_id having avg(salary) < 1500:\n"
